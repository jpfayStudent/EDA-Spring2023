...
#Create a map
ggplot() + 
  geom_sf(data=huc8_sf,fill='purple',color='gray') + 
  geom_sf(data=neuse_sf,fill='orange')

mapview(huc8_sf,col.regions='purple') + 
  mapview(neuse_sf,col.regions='orange',alpha=1)
```
...
triCo_sf <- counties_sf_utm %>% 
  filter(NAME %in% c('Chatham','Durham','Orange','Wake'))
  
#Grab the intersecting HUC_8s
triHucs_1 <- huc8_sf_utm[triCo_sf,]
mapview(triHucs_1)

triHucs_2 <- huc8_sf_utm %>% 
  st_filter(triCo_sf)
mapview(triHucs_2)

#Intersect the HUC_8s
truHucs_clipped = huc8_sf_utm %>% 
  st_intersection(triCo_sf)
mapview(truHucs_clipped)
...
counties_3k <- counties_sf_utm %>% 
  st_filter(the_site_counties)

mapview(counties_3k) + 
  mapview(the_site_counties,col.regions='red',alpha=0.1)
